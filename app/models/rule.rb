...
class Rule < ApplicationRecord
  ...
  def check_basic_rule_violations(pokemon, action)
    ...
    when 'ポケモンが瀕死になったら死亡扱い'
      violations << "#{pokemon.display_name}は既に死亡しているため、使用できません。" if action == :revive && pokemon.dead?
    when 'ニックネーム必須'
      violations << "#{pokemon.species}にニックネームをつけてください。" if pokemon.nickname.blank?
    end
  ...
  def check_duplicate_violations(pokemon)
    ...
    if enabled?
      ...
      violations << "#{pokemon.species}は既に捕獲済みです。重複ポケモン制限により捕獲できません。" if same_species_count.positive?
    end
