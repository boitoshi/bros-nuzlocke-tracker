...
class StrategyGuidesController < ApplicationController
  before_action :set_strategy_guide, only: %i[show increment_views toggle_like]

  def index
    @strategy_guides = StrategyGuide.published

    # 検索
    @strategy_guides = @strategy_guides.search(params[:search]) if params[:search].present?

    # ゲームタイトルでフィルタ
    @strategy_guides = @strategy_guides.by_game(params[:game_title]) if params[:game_title].present?

    # ガイドタイプでフィルタ
    @strategy_guides = @strategy_guides.by_type(params[:guide_type]) if params[:guide_type].present?

    # 難易度でフィルタ
    @strategy_guides = @strategy_guides.by_difficulty(params[:difficulty]) if params[:difficulty].present?
    ...
    # ソート
    @strategy_guides = case params[:sort]
                       when 'popular'
                         @strategy_guides.popular
                       when 'recent'
                         @strategy_guides.recent
                       else
                         @strategy_guides.recent
                       end
    ...
    @guide_types = StrategyGuide.guide_types.keys.map do |key|
      [StrategyGuide.new(guide_type: key).guide_type_display, key]
    end
